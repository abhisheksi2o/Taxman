"""Reconciliation views and item resolution shared by the HTTP API and the in-browser backend."""
...
from app.models.common import SOURCE_LABELS, Provenance, SourceType, ValueStatus, money, utcnow
...
def reconciliation_view(case: TaxCase) -> dict:
    return {"items": [i.model_dump(mode="json") for i in case.reconciliation_items], "matrix": matrix(case),
            "last_run": case.meta.last_reconciliation_at.isoformat() if case.meta.last_reconciliation_at else None}


def issues_view(case: TaxCase) -> dict:
...
def item_detail(case: TaxCase, item_id: str) -> dict:
...
def resolve_item(case: TaxCase, item_id: str, action: str, payload: dict[str, Any] | None, note: str | None) -> tuple[ReconciliationItem, str, str | None]:
    """Apply a resolution. Returns (item, audit summary, navigate hint). Does not run reconciliation – call
    case_ops.commit / refresh afterwards."""
    item = next((i for i in case.reconciliation_items if i.id == item_id), None)
    if item is None:
        raise ServiceError(404, "Item not found")
    action = action.upper()
    payload = payload or next((r.payload for r in item.suggested_resolutions if r.action == action), {}) or {}
    if action == "ADD_INCOME":
        head = payload.get("head")
        if not head:
            raise ServiceError(400, "payload.head required")
        try:
        ...
        for ev in item.evidence:  # keep the number traceable to the documents that revealed it
            if ev.document_id:
        ...
        if not eid or not case_ops.remove_entity(case, eid):
            raise ServiceError(400, "entity_id required / not found")
        summary = "Duplicate entry removed"
    elif action == "CONFIRM_ENTITY":
        if payload.get("prefer_source"):
            for t in case.tax_deducted:
                if t.source_type.value == payload["prefer_source"] and t.section.replace("-", "").upper() == str(payload.get("section", "")).upper() and any(t.id == e.entity_id for e in item.evidence):
        ...
        if not note:
            raise ServiceError(400, "A reason is required to dismiss an item")
        summary = "User dismissed the item"
    elif action in ("UPLOAD_DOCUMENT", "UPDATE_PROFILE"):
        return item, "", ("documents" if action == "UPLOAD_DOCUMENT" else "profile")
    else:
        raise ServiceError(400, "Unknown action")
    item.status = "DISMISSED" if action == "DISMISS" else "RESOLVED"
    item.resolution_note = note or summary
    item.resolved_at = utcnow()
    return item, summary, None


def reopen_item(case: TaxCase, item_id: str) -> ReconciliationItem:
    item = next((i for i in case.reconciliation_items if i.id == item_id), None)
    if item is None:
        raise ServiceError(404, "Item not found")
